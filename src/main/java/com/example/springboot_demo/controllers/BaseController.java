...
import com.example.springboot_demo.modules.users.service.interfaces.BaseServiceInterface;
...
public abstract class BaseController<E, R, C, U, Rp extends JpaRepository<E, Long> & JpaSpecificationExecutor<E>> {
    ...
    public ResponseEntity<?> pagination(HttpServletRequest request) {
        Map<String, String[]> parameters = request.getParameterMap();
        Page<E> entities = service.pagination(parameters);
    ...
    public ResponseEntity<?> delete(@PathVariable Long id) {
        try {
            ...
            return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR).body(
                    ApiResource.error("INTERNAL_SERVER_ERROR", "Có lỗi xảy ra trong quá trình xử lý",
                            HttpStatus.INTERNAL_SERVER_ERROR));
        }
    }
    ...
    public ResponseEntity<?> deleteMany(@RequestBody List<Long> ids) {
        try {
            service.deleteMutipleEntity(ids);
            return ResponseEntity.ok(ApiResource.message("Xóa bản ghi thành công", HttpStatus.OK));
